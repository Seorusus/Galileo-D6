...
$windowtitle = '';             // Window title (text only)
$defaultstatusmsg = $title;        // Default status message (text only)

$bodybgcolor  = "#A8CFEE";         // Background color of page body
...
$allowunknownusers = true;       // If set to false, any server authenticated but locally unknown user
...
$basedir = str_replace('/editor/core/filebrowser','',@dirname(__FILE__));    // Base directory = local directory
         ...
         // File section
            ...
            if (($act != "search") && ($allowedit || $allowshow) && checkExtension($file["name"]) && ($subdir != $trashcan)) {
               if (!$file["readonly"] && $allowedit) {
                  echo "<a href=$thisscript?act=edit&subdir=".rawurlencode($subdir)."&sortby=$sortby&file=".rawurlencode($file["name"])." onClick='selCli();return false;' onMouseOver='return statusMsg(\"".quoteJS($messages["edt8"])."\");' onMouseOut='return statusMsg(\"\");'>".(($useimages) ? "<img src=\"$imagesdir/$editimage\" border=0>" : $messages["edt10"])."</a> ";
               }
