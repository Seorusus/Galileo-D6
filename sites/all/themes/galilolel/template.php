...
if (arg(0) == 'taxonomy' && arg(1) == 'term' && is_numeric(arg(2))) {
    ...
    if (in_array($vid, array('16'))) {   // where 1 is the vid of adowner.
      $vars['template_file'] = 'page-adowner-vocab';
    }
  }
...
function galilolel_views_mini_pager($tags = array(), $limit = 10, $element = 0, $parameters = array(), $quantity = 9) {
  ...
  $li_previous = theme('pager_previous', (isset($tags[1]) ? $tags[1] :"<img src='/sites/all/themes/galilolel/images/next-horizontal.png' />"), $limit, $element, 1, $parameters);
...
//echo 'MSIE'; //// this doesnt not work with boost!
    $items[] = array(
...
function galilolel_menu_item($link, $has_children, $menu, $in_active_trail = FALSE, $extra_class = NULL) {
  ...
  if (!empty($link)) {
            ...
            $addclass = strstr($link, 'menu-hilight');
			if($addclass) {
			$menu_hilight  = "menu-hilight" ;
			}
    ...
    return '<li class="' . $menu_hilight.' ' .$class .'">'. $link . $menu ."</li>\n";
}

function galilolel_preprocess_views_exposed_form(&$vars) {
//dpm($vars['form']['#id']); 
//print_r($vars['form']['#id']); 
         // only alter the required form based on id
            if ($vars['form']['#id'] == 'views-exposed-form-adowner-by-taxonomy-index-page-2') {
              // Change the text on the submit button
              $vars['form']['submit']['#value'] = t('חפש');
...
function galilolel_preprocess_views_view(&$vars) {
      $view = $vars['view'];
	  
      // Add Javascript when the view with the name news is shown and the display id is your-display-id.
      if ($view->name == 'adowner_by_taxonomy_index' && $view->current_display == 'page_2') {
         drupal_add_js('jQuery(document).ready(function () { 
			 $(".True-promote-adowner").parents("table").addClass( "promote-adowner-group" ) ;
		 });', 'inline');
         drupal_add_js('jQuery(document).ready(function () { 
			 $(".False-promote-adowner").parents("table").addClass( "no-promote-adowner-group" ) ;
		 });', 'inline');
      }

      if ($view->name == 'adowner_by_taxonomy_index' && $view->current_display == 'page_1') {
         drupal_add_js('jQuery(document).ready(function () { 
			 $(".True-promote").parents("tr").addClass( "promote-adowner-group" ) ;
		 });', 'inline');
         drupal_add_js('jQuery(document).ready(function () { 
			 $(".False-promote").parents("tr").addClass( "no-promote-adowner-group" ) ;
		 });', 'inline');
      }
